...
class RegistrationEventIntegrationTest {
    @LocalServerPort
    private int port;
    ...
    void shouldUploadRegistrationEventToS3() {
        RegistrationEvent registrationEventRequest = RegistrationEventBuilder
                .withDefaultEventValues()
                .build();

        EventDAO expectedS3UploadEvent = EventDAOBuilder.withEvent(registrationEventRequest)
                .eventId(UUIDService.buildUUIDStringFromSeed(
                        registrationEventRequest.getConversationId() +
                                EventType.REGISTRATION_STARTED +
                                registrationEventRequest.getRegistrationEventDateTime())
                )
                .eventType(EventType.REGISTRATION_STARTED)
                .build();

        EventResponse actualResponseEvent = restTemplate.postForObject("http://localhost:" + port +
                "/registration", registrationEventRequest, EventResponse.class);
